...
for (fake_dyad in 1:nrow(pseudo_partners)){ 
  ...
  dplyr::filter(speaker_A==pseudo_partners$speaker_A[fake_dyad]) %>%
      speaker_A = pseudo_partners$speaker_A[fake_dyad] %>%
      fw_quantiles_A = fw_quantiles_A %>%
      
    temp_A = data.frame(speaker_A, fw_quantiles_A) %>%
    mutate(ident = 1:nrow(temp_A)) %>%

dplyr::filter(speakerB==pseudo_partner$next_fake_B[fake_dyad]) %>%
    speaker_B = pseudo_partners$speaker_b[fake_dyad] %>%
  
    temp_B = data.frame(speaker_B, fw_quantiles_B) %>%
