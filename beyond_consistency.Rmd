...
#Create surrogate language data through phase-randomization

```{r}
#Create surrogate data for speaker A

#split Speaker As by dyad_id
split_As = split(A_dfs, list(A_Dfs$dyad_id))

for (next_conv in split_conv)
  mutate(sur_fw_quantiles = FFTsurrogate(next_conv$fw_quantiles))

#Should we drop the non-necessary variables: function_words, fw_qunatiles, Segment etc.
