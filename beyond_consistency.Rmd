...
# we don't want to update their names QUITE yet -- this is partially why the pseudocode didn't work, sorry
...
# # this is actually not necessary here--"Segment" gives us time, so we can rename "Segment" to "time" if you want
...
speaker_A_tranformed = both_speakers %>% ungroup() %>%
...
df_final = full_join(speaker_A_tranformed, speaker_B_transformed)
...
# yes, you can---you can use arrange(time)
