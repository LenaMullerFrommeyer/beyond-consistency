...
# prepare conversations Speaker B for RQA
B_dfs = B_dfs %>% ungroup() %>%
...
for (next_conv in split_conv){
  ...
  rqa_for_conv = crqa(ts1=next_conv$fw_quantiles_A,
                      ...
                      mindiagline=2,
                      minvertline=2, 
                      tw=0, #1 is correct for the drp, right?
                      whiteline=FALSE,
                      recpt=FALSE)
...
#has too many random effects - uncorrelated random intercept and random slope

h1_analyses <- lmer(RR ~ conv.type + (1|speaker_code) + (0+ conv.type | speaker_code), data = h1_data, REML = FALSE)

#still has too many random effects - only random intercept

h1_analyses <- lmer(RR ~ conv.type + (1|speaker_code), data = h1_data, REML = FALSE)
summary(h1_analyses)

#for now, model doesn't include the cond, but there was only a condition for the conversations, the monologue was identital for all participants - so is it necessary to add it and if yes, where/how should we add it?
