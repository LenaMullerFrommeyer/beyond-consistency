...
plot_drp_real = ggplot(data=drp_real,
                       aes(y=rr,
                           x=raw,
                           colour=as.factor(condition))) +
  geom_smooth() +
  ggtitle("Diagonal recurrence profiles by conversation type") +
  xlab("Lag (in turns)") +
  ylab("Rate of recurrence (RR)") +
  labs(colour="Conversation") +
  scale_color_manual(breaks=levels(drp_real$condition),
                     labels=c("Personal",
                              "Conflict"),
                     values=c("#5ab4ac",
                              "#d8b365"))
...
# save the file
ggsave(filename="./figures/bc-drp_real.png",
       plot=plot_drp_real,
       width=6,
       height=5,
       units="in",
