...
```{r read-in partner-wise-baseline}

partner-wise-baseline = read.csv('./data/partner_wise_baseline-partner_IDs.csv', header = TRUE, sep=",")
...
split_drp_real = split(drp_real, list(drp_real$dyad_id))


#reverse the order for all dyads and conversations if mean RR negative lags < mean RR positive lags

## AP: Slight error in first 'for' loop here -- just make sure 
## that they are ranges or lists, not single values. In this
## case, we can even go ahead and get them to print us the
## whole list of unique IDs!


for (next_dyad in unique(split_drp_real)) { 
  
  ## AP: Same with the second 'for' loop. Another tip is that you might
  ## want to do to save time and processing speed by first filtering
  ## by dyad the the outer 'for' loop and then filtering by conversation
  ## in the inner 'for' loop
  
  # grab the next dyad's data
  next_dyad_df =  drp_real_coll %>%
      dplyr::filter(dyad_id==next_dyad) 
  
  # cycle through their conversations
  
  ## AP: We can again use the 'unique' list here!
  
  for (next_conv in unique(next_dyad_df$conv.no)) {
    
    ## AP: You've got the right idea! This would be a bit easier to go
    ## and use the 'temp' variables directly as single values instead
    ## of df -- like this:
    ...
    temp_1 = drp_real_coll %>%
      ...
      .$mean_rr # this allows you to return only a single variable
    ...
    ## AP: Instead of doing an 'if_else' in dplyr, I would recommend doing
    ## an 'if/else' statement that will do the 'rev' if temp_1 > temp_2
...
#check each column if it is a factor - if yes, convert to numeric
...
H2_st = lmer(rr ~ condition + conv.no + ot1 + ot2 + ot1.ot2 + condition.ot1 + condition.ot2 + condition.ot1.ot2 + (1 | dyad_id), data=drp_st, REML=FALSE)
...
# raw maximal random-effects model
H2_sample_st = lmer(rr ~ condition + conv.no + ot1 + ot2 + ot1.ot2 + condition.ot1 + condition.ot2 + condition.ot1.ot2 + (1 | speaker_A) + (1 | speaker_B), data=drp_sample_st, REML=FALSE)
