...
```
***
## Creation of surrogate baselines
...
for (next_conv in split_h2_b_sur){
  ...
  if (dim(next_conv)[1] > 1){
    ...
    for (run in c(1:10)){
      ...
      # I don't even know if that is the right way to do it, but I think it would work if I could just assign these         # identifiers (e.g. A11 and B29 (both with identifier 155) would be paired)
...
```
***
###Cross-recurrence quantification analyses partner-wise surrogates
...
# split dataframe by conversation and dyad ID
split_sw_sur = split(sample_wise_baseline, list(sample_wise_baseline$dyad_id,
                                  sample_wise_baseline$cond, sample_wise_baseline$run))

# cycle through all dyads
drp_results_sw_sur = data.frame()
for (next_conv in split_sw_sur) {
  
  # only proceed if we have data for this conversation
  if (dim(next_conv)[1]>9){
    # calculate diagonal recurrence profile using categorical recurrence
    drp_for_conv_sur = drpdfromts(next_conv$sur_fw_quantiles_A, next_conv$sur_fw_quantiles_B, 
                              ws = wsz, datatype="categorical")
    
    # save plot-level information to dataframe
    dyad_id = unique(next_conv$dyad_id)
    cond = unique(next_conv$cond)
    run = unique(next_conv$run)
    drps = data.frame(raw = timeVals$raw, 
                      ot1 = timeVals$ot1, 
                      ot2  = timeVals$ot2, 
                      rr = drp_for_conv$profile) %>%
    ...
    drp_results_sw_sur = rbind.data.frame(drp_results_sw_sur, drps)
    }
}
```
...
##Preparing data for final analyses
...
#'s create a new dataframe with all standardized variables. This allows us to interpret the #resulting values as effect sizes (see Keith, 2005, *Multiple regression and beyond*).
