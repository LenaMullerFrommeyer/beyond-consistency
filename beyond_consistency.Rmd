...
#calculate 95%CI for data
library(emmeans)
CI_easy_raw <- confint(planned_analyses_raw)
CI_easy_raw
CI_raw <- with(summary(lsmeans(planned_analyses_raw,spec="conv.type")),cbind(lower.CL,upper.CL))
CI_raw
```
...
CI_easy_st <- confint(planned_analyses_st)
CI_easy_st
CI_st <- with(summary(lsmeans(planned_analyses_st,spec="conv.type")),cbind(lower.CL,upper.CL))
CI_st
...
library(simr)
fit <- planned_analyses_raw
power <- powerSim(fit,nsim = 200)
power

#Find out which combination of speakers and conv.types are appropriate
part <- unique(analysis_df$speaker_code)

#calculate the power of 100 samples of 40 participants
pow = list()
for (i in 1:100) {
print(i)
selectionpart <- sample(part,30)
df_2 <- analysis_df[which(analysis_df$speaker_code %in% selectionpart), ]
fit <- lmer(RR ~ conv.type + (1|speaker_code),
                             data = df_2, REML = FALSE)
...
``` {r calculate effect size d}
#subset mon
mon <- subset(analysis_df, conv.type==0.5)
con <- subset(analysis_df, conv.type == -0.5)
M_RR_mon <- mean(mon$RR)
M_RR_con <- mean(con$RR)

#calculate effect size following Westfall, Judd & Kenny, 2014
effect_size = ((M_RR_mon) - (M_RR_con))/(sqrt(5.267+23.621)) #variances of random intercepts
effect_size
