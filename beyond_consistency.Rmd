...
for (fake_dyad in 1:nrow(partner_wise_baseline)){ 
  ...
  if(nrow(temp_data)>1){
    ...
    next_data_line = data.frame(speaker_A,
                                ...
                                cond,
                                crqa_for_sur[1:9])
    crqa_partner = rbind.data.frame(crqa_partner,next_data_line)
  }
}
...
#drp for partner-wise surrogate body
...
dyad = unique(sample_wise_baseline$dyad_id)

# cycle through all the pseudopartners
for (i in 1:length(unique(sample_wise_baseline$dyad_id))){
  
  for (run in 1:max(sample_wise_baseline$run)) {
    
    # get next Speaker A data
  temp_AB = sample_wise_baseline %>% ungroup() %>%
    dplyr::filter(dyad_id==dyad[i] && run == run) %>%
